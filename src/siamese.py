...
def build_tracking_graph(final_score_sz, design, env):
    ...
    istrain = tf.placeholder(tf.int32, 1, name='istrain')
    ...
    template_z, templates_x, p_names_list, p_val_list = _create_siamese(os.path.join(env.root_pretrained,design.net), x_crops, z_crops, istrain)
...
def _match_templates(net_z, net_x, params_names_list, params_values_list, istrain):
    # finalize network
    # z, x are [B, H, W, C]
    ...
    # final is [B, Hf, Wf, 1]
    if _bnorm_adjust:
        net_final = tf.layers.batch_normalization(net_final, training=istrain, trainable=True)
